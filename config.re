...
void config_parse_line(char *input_str, int line_number) {
  ...
  *      { DIAG("ignoring unknown config line: '%s'", input_str); return; }
  ...
  "UserAdder:"               [ \t]* @start [0-9a-fA-F]{128} @end { _config_user_adder(); }
  "UserExtender:"            [ \t]* @start [0-9a-fA-F]{128} @end { _config_user_extender(); }
  "UserNormal:"              [ \t]* @s1 [0-9]+ @e1 [ \t]+ @start [0-9a-fA-F]{128} @end { *e1=0; _config_user_normal(strtol(s1, 0, 10)); }
